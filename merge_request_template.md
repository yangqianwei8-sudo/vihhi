...
**若本 MR 涉及 `plan_management`（计划管理）或相关页面/API/权限/工作流**，请勾选并确认已按 [计划模块安全回归清单](backend/apps/plan_management/docs/plan_security_regression.md) 执行：
...
**未涉及计划模块**：本 MR 不修改 `backend/apps/plan_management` 及计划相关调用，上述勾选不适用。
