...
from backend.apps.project_center.models import ProjectTeamNotification, Project, ProjectInitiationApproval
...
# 2. 检查项目立项审批记录
print("\n2. 项目立项审批记录:")
approvals = ProjectInitiationApproval.objects.all().order_by('-created_time')[:5]
print(f"   总数: {approvals.count()}")
for approval in approvals:
    print(f"   - 项目: {approval.project.name}")
    print(f"     状态: {approval.get_status_display()}")
    print(f"     提交人: {approval.submitted_by.username}")
    if approval.supervisor:
        print(f"     直接上级: {approval.supervisor.username}")
    if approval.technical_manager:
        print(f"     技术部经理: {approval.technical_manager.username}")
    print()
...
# 5. 检查项目立项相关的通知
print("\n5. 项目立项相关通知:")
initiation_notifications = ProjectTeamNotification.objects.filter(
    context__notification_type__in=['project_initiation_approval', 'project_initiation_approved', 'project_initiation_rejected']
).order_by('-created_time')
print(f"   总数: {initiation_notifications.count()}")
for n in initiation_notifications[:5]:
    print(f"   - {n.title}")
    print(f"     接收人: {n.recipient.username}")
    print(f"     项目: {n.project.name if n.project else 'N/A'}")
    print(f"     时间: {n.created_time}")
