"""
结算管理模块的 API 视图（使用本应用 models/serializers/services）
"""
...
from .service_fee_services import (
...
class ServiceFeeSettlementSchemeViewSet(viewsets.ModelViewSet):
    ...
    def by_contract(self, request):
        ...
        if not contract_id:
            return Response({'error': '缺少contract_id参数'}, status=status.HTTP_400_BAD_REQUEST)
        try:
    ...
    def calculate_by_scheme_id(self, request):
        ...
        try:
            ...
            return Response({'error': '结算方案不存在或已禁用'}, status=status.HTTP_404_NOT_FOUND)
        if saving_amount is not None:
            saving_amount = Decimal(str(saving_amount))
        if service_area is not None:
            service_area = Decimal(str(service_area))
