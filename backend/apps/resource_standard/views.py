...
RESOURCE_MANAGEMENT_MENU = [
    {
        'id': 'resource_standard_home',
        'label': '资源管理首页',
        'icon': '🏠',
        'url_name': 'resource_standard:resource_standard_home',
        'permission': 'resource_center.view',
    },
    {
        'id': 'standard_management',
        'label': '审查标准',
        'icon': '📐',
    ...
    },
    {
        ...
        'children': [
            {
                'id': 'material_price_list',
                'label': '价格列表',
                'icon': '📋',
                'url_name': 'resource_standard:material_price_list',
                'permission': 'resource_center.manage_library',
            },
        ]
    },
    {
        ...
        'children': [
            {
                'id': 'cost_indicator_list',
                'label': '指标列表',
                'icon': '📋',
                'url_name': 'resource_standard:cost_indicator_list',
                'permission': 'resource_center.manage_library',
            },
        ]
    },
    {
        ...
        'children': [
            ...
            {
                'id': 'opinion_template_list',
                'label': '意见模板',
                'icon': '📝',
                'url_name': 'resource_standard:opinion_template_list',
                'permission': 'resource_center.manage_library',
            },
        ]
    },
        ...
        'children': [
            {
                ...
                'label': '风险案例',
                'icon': '⚠️',
                'url_name': 'resource_standard:risk_case_list',
                'permission': 'resource_center.view',
            },
            {
                'id': 'technical_solution_list',
                'label': '技术方案',
                'icon': '🔧',
                'url_name': 'resource_standard:technical_solution_list',
                'permission': 'resource_center.view',
            },
        ]
    },
    {
        ...
        'children': [
            ...
            {
                'id': 'system_parameter_list',
                'label': '系统参数',
                'icon': '🔧',
                'url_name': 'resource_standard:system_parameter_list',
                'permission': 'resource_center.data_maintenance',
            },
        ]
    },
...
def _build_resource_management_sidebar_nav(permission_set, request_path=None, active_id=None):
    """生成资源管理左侧菜单（统一格式）"""
    # 使用统一的菜单构建函数
    return _build_unified_sidebar_nav(RESOURCE_MANAGEMENT_MENU, permission_set, active_id=active_id)
...
def _context(page_title, page_icon, description, summary_cards=None, sections=None, request=None):
    ...
    if request and request.user.is_authenticated:
        ...
        context['module_sidebar_nav'] = _build_resource_management_sidebar_nav(permission_set, request.path)
    else:
...
@login_required
def resource_standard_home(request):
    """资源标准首页"""
    permission_codes = get_user_permission_codes(request.user)
    
    # 权限检查
    if not _permission_granted('resource_center.view', permission_codes):
        messages.error(request, '您没有权限访问资源标准')
    ...
    try:
        # 标准统计
        if _permission_granted('resource_center.manage_library', permission_codes):
            try:
                total_standards = Standard.objects.count()
                active_standards = Standard.objects.filter(status='effective').count()
                
                summary_cards.append({
                    'label': '审查标准',
                    'icon': '📐',
                    'value': str(total_standards),
                    'subvalue': f'生效 {active_standards} 个',
                    'url': reverse('resource_standard:standard_list'),
                    'variant': 'info'
                })
            except Exception:
                pass
        ...
        if _permission_granted('resource_center.manage_library', permission_codes):
            try:
                total_materials = MaterialPrice.objects.count()
                active_materials = MaterialPrice.objects.filter(is_active=True).count()
                
                summary_cards.append({
                    'label': '材料价格',
                    'icon': '💰',
                    'value': str(total_materials),
                    'subvalue': f'启用 {active_materials} 个',
                    'url': reverse('resource_standard:material_price_list'),
                    'variant': 'info'
                })
            except Exception:
                pass
        ...
        if _permission_granted('resource_center.manage_library', permission_codes):
            try:
                total_indicators = CostIndicator.objects.count()
                
                summary_cards.append({
                    'label': '成本指标',
                    'icon': '📊',
                    'value': str(total_indicators),
                    'subvalue': '成本指标数量',
                    'url': reverse('resource_standard:cost_indicator_list'),
                    'variant': 'info'
                })
            except Exception:
                pass
        ...
        # 知识库统计
        if _permission_granted('resource_center.view', permission_codes):
            try:
                total_risk_cases = RiskCase.objects.count()
                total_solutions = TechnicalSolution.objects.count()
                
                summary_cards.append({
                    'label': '知识库',
                    'icon': '📚',
                    'value': str(total_risk_cases + total_solutions),
                    'subvalue': f'风险案例 {total_risk_cases} 个 · 技术方案 {total_solutions} 个',
                    'url': reverse('resource_standard:risk_case_list'),
                    'variant': 'info'
                })
            except Exception:
                pass
    ...
    context = _context(
        page_title="资源标准",
        page_icon="🗂️",
        description="管理审查标准、材料价格、成本指标和知识库",
        summary_cards=summary_cards,
        sections=sections,
        request=request,
    )
    
    return render(request, "resource_standard/home.html", context)
