...
# ==================== 权限检查工具函数 =====================
def check_permission_or_redirect(request, permission_code, redirect_url=None, error_message=None):
    """
    检查用户权限，如果没有权限则重定向或返回错误
    
    参数:
        request: Django request 对象
        permission_code: 权限代码
        redirect_url: 重定向URL（如果为None，则返回JSON错误）
        error_message: 错误消息
    
    返回:
        HttpResponse 或 None（如果有权限则返回None）
    """
    ...
    if not _permission_granted(permission_code, permission_codes):
        if error_message is None:
            error_message = f'您没有权限执行此操作'
