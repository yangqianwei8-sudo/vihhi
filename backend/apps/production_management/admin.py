...
from backend.apps.production_management.models import (
    ...
    # BusinessContract,  # 已从后台管理中移除
    # BusinessPaymentPlan,  # 已从后台管理中移除
    StructureType, DesignUnitCategory, Project,
    ComprehensiveAdjustmentCoefficient, AIAdvisorSelectedSolution
)
...
class StructureTypeAdmin(LinkAdminMixin, BaseModelAdmin):
    ...
    def contract_count(self, obj):
        """显示使用该结构形式的合同数量（已从后台管理移除）"""
        # 商务合同已从后台管理中移除，请使用前端管理页面
...
@admin.register(AIAdvisorSelectedSolution)
class AIAdvisorSelectedSolutionAdmin(BaseModelAdmin):
    """AI顾问选中方案管理"""
    list_display = ('solution_title', 'user', 'savings', 'risk_level', 'created_time')
    list_filter = ('risk_level', 'created_time', 'service_type_id')
    search_fields = ('solution_title', 'problem_description', 'user__username')
    readonly_fields = ('created_time',)
    ordering = ('-created_time',)
    
    fieldsets = (
        ('基本信息', {
            'fields': ('user', 'solution_title', 'solution_index')
        }),
        ('问题与方案', {
            'fields': ('problem_description', 'constraints', 'solution_description')
        }),
        ('方案评估', {
            'fields': ('savings', 'risk_level', 'advantages', 'considerations')
        }),
        ('关联信息', {
            'fields': ('service_type_id', 'profession_code', 'budget_impact')
        }),
        ('扩展信息', {
            'fields': ('metadata',),
            'classes': ('collapse',)
