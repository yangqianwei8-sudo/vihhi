...
def geocode_address(request):
    """
    地理编码API：将地址字符串转换为结构化地址信息和经纬度坐标
    
    请求参数:
    - address: 地址字符串（必需），如"北京市朝阳区阜通东大街6号"
    - city: 城市名称或城市编码（可选），限制搜索范围，提高准确性
    ...
    try:
        ...
        # 调用高德地图地理编码服务
        try:
            result = amap_service.geocode(address, city if city else None)
        except Exception as e:
            logger.exception(f'高德地图API调用异常: {str(e)}')
            return Response({
                'success': False,
        ...
        return Response({
            'success': False,
            'message': '未找到匹配的地址信息，请检查地址是否正确'
        }, status=status.HTTP_404_NOT_FOUND)
    
    except Exception as e:
        logger.exception('地理编码失败: %s', str(e))
        return Response({
            'success': False,
...
@api_view(['POST'])
@permission_classes([permissions.IsAuthenticated])
def opportunity_batch_delete(request):
    """
    批量删除商机API
    
    请求参数:
    {
        "opportunity_ids": [1, 2, 3]  # 商机ID列表
    }
    ...
    try:
        ...
        # 获取商机列表
        opportunities = BusinessOpportunity.objects.filter(id__in=opportunity_ids)
        
        # 权限检查：普通用户只能删除自己负责的商机
        if not _permission_granted('customer_management.opportunity.view_all', permission_set):
            opportunities = opportunities.filter(business_manager=request.user)
        ...
        for opportunity in opportunities:
            try:
                ...
                import logging
                logger = logging.getLogger(__name__)
                logger.exception(f'删除商机失败: {opportunity.id}')
                failed_count += 1
        ...
        return Response({
            ...
            'message': f'成功删除 {deleted_count} 个商机' + (f'，{failed_count} 个删除失败' if failed_count > 0 else '')
        }, status=status.HTTP_200_OK)
        
    except Exception as e:
        import logging
        logger = logging.getLogger(__name__)
        logger.exception('批量删除商机失败')
        return Response({
            'success': False,
...
@api_view(['POST'])
@permission_classes([permissions.IsAuthenticated])
def opportunity_batch_update(request):
    """
    批量修改商机API
    ...
    {
        "opportunity_ids": [1, 2, 3],  # 商机ID列表
        "status": "won",  # 可选，要修改的状态
        "urgency": "urgent"  # 可选，要修改的紧急程度
    }
    ...
    try:
        ...
        if not new_status and not new_urgency:
            return Response({
                'success': False,
                'message': '请提供要修改的字段（status 或 urgency）'
            }, status=status.HTTP_400_BAD_REQUEST)
        
        # 获取商机列表
        opportunities = BusinessOpportunity.objects.filter(id__in=opportunity_ids)
        
        # 权限检查：普通用户只能修改自己负责的商机
        if not _permission_granted('customer_management.opportunity.view_all', permission_set):
            opportunities = opportunities.filter(business_manager=request.user)
        ...
        # 验证状态值
        if new_status:
            valid_statuses = [choice[0] for choice in BusinessOpportunity.STATUS_CHOICES]
        ...
        # 验证紧急程度值
        if new_urgency:
            valid_urgencies = [choice[0] for choice in BusinessOpportunity.URGENCY_CHOICES]
            if new_urgency not in valid_urgencies:
                return Response({
                    'success': False,
                    'message': f'无效的紧急程度值：{new_urgency}'
                }, status=status.HTTP_400_BAD_REQUEST)
        ...
        for opportunity in opportunities:
            try:
                ...
                if new_urgency:
                    opportunity.urgency = new_urgency
                opportunity.save()
                updated_count += 1
            except Exception as e:
                import logging
                logger = logging.getLogger(__name__)
                logger.exception(f'修改商机失败: {opportunity.id}')
                failed_count += 1
        
        update_fields = []
        if new_status:
            status_label = dict(BusinessOpportunity.STATUS_CHOICES).get(new_status, new_status)
            update_fields.append(f'状态为{status_label}')
        if new_urgency:
            urgency_label = dict(BusinessOpportunity.URGENCY_CHOICES).get(new_urgency, new_urgency)
            update_fields.append(f'紧急程度为{urgency_label}')
        
        message = f'成功修改 {updated_count} 个商机的{"和".join(update_fields)}'
        if failed_count > 0:
            message += f'，{failed_count} 个修改失败'
        ...
        import logging
        logger = logging.getLogger(__name__)
        logger.exception('批量修改商机失败')
        return Response({
            'success': False,
            'message': f'批量修改失败：{str(e)}'
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)


@api_view(['POST'])
@permission_classes([permissions.IsAuthenticated])
def opportunity_batch_export(request):
    """
    批量导出商机API
    
    请求参数:
    {
        "opportunity_ids": [1, 2, 3]  # 商机ID列表（可选，不提供则导出所有）
    }
    ...
    try:
        ...
        # 获取商机列表
        if opportunity_ids:
            opportunities = BusinessOpportunity.objects.filter(id__in=opportunity_ids).select_related(
        ...
        opportunities = opportunities.order_by('-created_time')
        ...
        # 设置表头
        headers = [
            '商机编号', '商机名称', '关联客户', '负责商务', '商机状态',
            '预计金额（万元）', '成功概率（%）', '加权金额（万元）', '紧急程度',
            '预计签约时间', '创建时间', '创建人'
        ]
        ...
        # 填充数据
        status_map = dict(BusinessOpportunity.STATUS_CHOICES)
        urgency_map = dict(BusinessOpportunity.URGENCY_CHOICES)
        
        for row, opp in enumerate(opportunities, 2):
            ws.cell(row=row, column=1, value=opp.opportunity_number or '')
            ws.cell(row=row, column=2, value=opp.name or '')
            ws.cell(row=row, column=3, value=opp.client.name if opp.client else '')
            ws.cell(row=row, column=4, value=opp.business_manager.get_full_name() if opp.business_manager else '')
            ws.cell(row=row, column=5, value=status_map.get(opp.status, opp.status))
            ws.cell(row=row, column=6, value=float(opp.estimated_amount) if opp.estimated_amount else 0)
            ws.cell(row=row, column=7, value=opp.success_probability)
            ws.cell(row=row, column=8, value=float(opp.weighted_amount) if opp.weighted_amount else 0)
            ws.cell(row=row, column=9, value=urgency_map.get(opp.urgency, opp.urgency))
            ws.cell(row=row, column=10, value=opp.expected_sign_date.strftime('%Y-%m-%d') if opp.expected_sign_date else '')
            ws.cell(row=row, column=11, value=opp.created_time.strftime('%Y-%m-%d %H:%M:%S') if opp.created_time else '')
            ws.cell(row=row, column=12, value=opp.created_by.get_full_name() if opp.created_by else '')
        
        # 调整列宽
        column_widths = [18, 30, 20, 15, 12, 15, 12, 15, 12, 15, 18, 15]
        ...
        filename = timezone.now().strftime('商机列表_%Y%m%d_%H%M%S.xlsx')
