...
class AgentConversation(models.Model):
    """Agent对话会话"""
    title = models.CharField(max_length=200, verbose_name='对话标题', help_text='对话的标题或主题')
    description = models.TextField(blank=True, verbose_name='对话描述', help_text='对话的简要描述')
    
    # 关联用户
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='agent_conversations', verbose_name='用户')
    
    # 对话元数据（JSON格式存储额外信息）
    metadata = models.JSONField(default=dict, blank=True, verbose_name='元数据', help_text='存储对话的额外信息，如模型类型、参数等')
    
    # 状态
    is_active = models.BooleanField(default=True, verbose_name='是否活跃', help_text='是否正在进行的对话')
    is_archived = models.BooleanField(default=False, verbose_name='是否归档', help_text='是否已归档')
    ...
    def update_last_message_time(self):
        """更新最后消息时间"""
        last_message = self.messages.order_by('-created_time').first()
        if last_message:
            self.last_message_time = last_message.created_time
...
class AgentMessage(models.Model):
    ...
    # 消息元数据（JSON格式存储额外信息，如token数、模型名称等）
    metadata = models.JSONField(default=dict, blank=True, verbose_name='元数据', help_text='存储消息的额外信息')
    
    # 消息顺序
    sequence = models.IntegerField(default=0, verbose_name='消息顺序', help_text='消息在对话中的顺序')
    ...
    def __str__(self):
        return f"{self.conversation.title} - {self.get_role_display()} - {self.content[:50]}"
