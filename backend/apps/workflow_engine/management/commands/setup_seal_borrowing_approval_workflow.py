...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            content_type = ContentType.objects.get_for_model(SealBorrowing)
            model_name = content_type.model.lower()  # 'sealborrowing'
            ...
            self.stdout.write('  请手动在后台管理中配置适用模型为：sealborrowing')
        ...
        node1, _ = ApprovalNode.objects.get_or_create(
            ...
            defaults={
                ...
                'can_transfer': False,
                'timeout_hours': 24,
                'description': '申请人所在部门的经理审批印章借用申请'
            }
        )
        # 更新节点配置（如果已存在）
        if not _:
            ...
            node1.description = '申请人所在部门的经理审批印章借用申请'
            node1.save()
        ...
        # 显示流程配置摘要
        self.stdout.write('\n' + '='*60)
        self.stdout.write(self.style.SUCCESS('印章借用审批流程配置完成！'))
        ...
        self.stdout.write('  ApprovalEngine.start_approval(workflow, seal_borrowing, user, comment)')
        self.stdout.write('\n注意事项：')
        self.stdout.write('- 每个节点审批超时时间为24小时')
        self.stdout.write('- 审批过程中可以驳回，驳回后流程终止')
        self.stdout.write('- 审批过程中可以撤回（如果流程配置允许）')
        self.stdout.write('- 适用模型：sealborrowing（印章借用）')
        self.stdout.write('='*60)
