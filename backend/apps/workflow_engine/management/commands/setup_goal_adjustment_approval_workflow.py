...
class Command(BaseCommand):
    help = '配置目标调整申请审批流程：申请人 -> 总经理'

    def handle(self, *args, **options):
        self.stdout.write('开始配置目标调整申请审批流程...')

        # 获取或创建审批流程模板
        workflow, created = WorkflowTemplate.objects.get_or_create(
            code='goal_adjustment_approval',
            defaults={
                'name': '目标调整申请审批流程',
                'description': '目标调整申请操作的审批流程',
                'category': '计划管理',
                'status': 'active',
                'allow_withdraw': True,
                'allow_reject': True,
                ...
                'form_filter_conditions': {
                    'strategicgoal': ['goaladjustment']  # 仅适用于目标调整申请表单
                },
