...
class Command(BaseCommand):
    help = '配置借款申请审批流程：申请人 -> 部门经理 -> 总经理，抄送出纳员'

    def handle(self, *args, **options):
        self.stdout.write('开始配置借款申请审批流程...')
        
        # 检查并创建出纳员角色（如果不存在）
        cashier_role, cashier_created = Role.objects.get_or_create(
            code='cashier',
            defaults={
                'name': '出纳员',
                'description': '负责现金收付、银行结算等出纳工作',
                'is_active': True,
            }
        )
        if cashier_created:
            self.stdout.write(self.style.SUCCESS(f'✓ 创建出纳员角色：{cashier_role.name} (code: {cashier_role.code})'))
        else:
            self.stdout.write(f'✓ 出纳员角色已存在：{cashier_role.name} (code: {cashier_role.code})')
        ...
        workflow, created = WorkflowTemplate.objects.get_or_create(
            ...
            defaults={
                'name': '借款申请审批流程',
                'description': '借款申请的审批流程：申请人 -> 部门经理 -> 总经理，抄送出纳员',
                'category': '行政管理',
                'status': 'active',
                'allow_withdraw': True,
                'allow_reject': True,
        ...
        try:
            ...
            model_name = content_type.model.lower()  # 'loanapplication'
            ...
            self.stdout.write('  请手动在后台管理中配置适用模型为：loanapplication')
        
        # 更新流程描述
        if not created:
            workflow.description = '借款申请的审批流程：申请人 -> 部门经理 -> 总经理，抄送出纳员'
            workflow.save()
        ...
        if not _:
            ...
            node1.description = '申请人所在部门的经理审批借款申请'
            node1.save()
        ...
        node2, _ = ApprovalNode.objects.get_or_create(
            ...
            defaults={
                'name': '总经理审批',
                'node_type': 'approval',
                'approver_type': 'role',  # 使用角色类型
                'approval_mode': 'single',  # 单人审批
                'is_required': True,
                'can_reject': True,
                'can_transfer': False,
                'timeout_hours': 24,
                'description': '总经理审批借款申请'
            }
        )
        # 更新节点配置（如果已存在）
        if not _:
        ...
        if general_manager_role:
            ...
            self.stdout.write(f'  审批人角色：{general_manager_role.name} (code: {general_manager_role.code})')
        else:
            self.stdout.write(self.style.WARNING(f'⚠ 创建/更新节点2：{node2.name}'))
            self.stdout.write(self.style.ERROR('  ⚠ 警告：未找到总经理角色（code: general_manager），请手动在后台管理中配置审批人角色'))
        ...
        # 查找出纳员角色用户（用于抄送）
        cashier_users = User.objects.filter(roles=cashier_role, is_active=True) if cashier_role else []
        
        # 显示流程配置摘要
        self.stdout.write('\n' + '='*60)
        self.stdout.write(self.style.SUCCESS('借款申请审批流程配置完成！'))
        ...
        for i, node in enumerate(workflow.nodes.all().order_by('sequence'), 1):
            approver_info = '未配置'
            if node.approver_type == 'creator_manager':
                approver_info = '申请人上级（自动获取）'
                ...
                users = ', '.join([u.get_full_name() or u.username for u in node.approver_users.all()])
        ...
        self.stdout.write('1. 申请人提交借款申请')
        self.stdout.write('2. 部门经理审批（必须通过）')
        self.stdout.write('3. 总经理审批（必须通过）')
        self.stdout.write('4. 审批完成，借款申请生效')
        if cashier_users.exists():
            cashier_names = ', '.join([u.get_full_name() or u.username for u in cashier_users])
            self.stdout.write(f'5. 抄送出纳员：{cashier_names}')
        else:
            self.stdout.write('5. 抄送出纳员（需要在审批流程中配置通知，当前没有用户被分配出纳员角色）')
        self.stdout.write('\n使用方法：')
        self.stdout.write('在借款申请创建视图中调用：')
        self.stdout.write('  from backend.apps.workflow_engine.services import ApprovalEngine')
        self.stdout.write('  ApprovalEngine.start_approval(workflow, loan_application, user, comment)')
        self.stdout.write('\n注意事项：')
        self.stdout.write('- 每个节点审批超时时间为24小时')
        self.stdout.write('- 审批过程中可以驳回，驳回后流程终止')
        self.stdout.write('- 审批过程中可以撤回（如果流程配置允许）')
        self.stdout.write('- 适用模型：loanapplication（借款申请）')
        self.stdout.write('- 抄送功能需要在审批流程服务中实现通知逻辑')
        self.stdout.write('- 出纳员角色已创建（code: cashier），请为相关用户分配此角色')
        self.stdout.write('='*60)
