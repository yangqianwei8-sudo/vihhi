...
class Command(BaseCommand):
    help = '生成日计划创建待办事项（每天下午5点执行，截止时间为明天下午18点）'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'开始生成日计划创建待办（时间：{timezone.now()}）...')
        ...
        try:
            ...
            if not dry_run:
                ...
                self.stdout.write(self.style.SUCCESS(f'成功生成 {len(todos)} 个日计划创建待办'))
            else:
                self.stdout.write(self.style.WARNING(f'试运行模式：将为所有活跃员工生成日计划创建待办（截止时间：{deadline.strftime("%Y-%m-%d %H:%M")}）'))
            
        except Exception as e:
            logger.error(f"生成日计划创建待办失败: {str(e)}", exc_info=True)
