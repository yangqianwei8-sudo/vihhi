...
class PaymentApplication(models.Model):
    ...
    contract = models.ForeignKey(
        'production_management.BusinessContract',
    ...
    application_amount = models.DecimalField(max_digits=12, decimal_places=2, verbose_name='申请金额')
    application_date = models.DateField(verbose_name='申请日期')
    expected_date = models.DateField(null=True, blank=True, verbose_name='期望回款日期')
    ...
    submitted_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name='submitted_payment_applications',
        verbose_name='提交人'
    )
    submitted_time = models.DateTimeField(null=True, blank=True, verbose_name='提交时间')
    ...
    approver = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name='approved_payment_applications',
        verbose_name='审核人'
    )
    approved_time = models.DateTimeField(null=True, blank=True, verbose_name='审核时间')
    review_comment = models.TextField(blank=True, verbose_name='审核意见')
    ...
    created_by = models.ForeignKey(
        User,
        on_delete=models.PROTECT,
        related_name='created_payment_applications',
        verbose_name='创建人'
    )
    created_time = models.DateTimeField(default=timezone.now, verbose_name='创建时间')
    updated_time = models.DateTimeField(auto_now=True, verbose_name='更新时间')
    ...
    def __str__(self):
        return f"{self.application_number} - ¥{self.application_amount}"
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.application_number:
            ...
            from datetime import datetime
            current_year = datetime.now().year
            max_application = PaymentApplication.objects.filter(
                application_number__startswith=f'PAY-APP-{current_year}-'
            ).aggregate(max_num=Max('application_number'))['max_num']
            
            if max_application:
                try:
                    seq = int(max_application.split('-')[-1]) + 1
                except (ValueError, IndexError):
                    seq = 1
            else:
                seq = 1
            
            self.application_number = f'PAY-APP-{current_year}-{seq:04d}'
