...
class OutputValueStageAdmin(BaseModelAdmin):
    """产值阶段管理"""
    list_display = ('name', 'code', 'stage_type', 'stage_percentage', 'base_amount_type', 'order', 'is_active', 'created_time')
    list_filter = ('stage_type', 'base_amount_type', 'is_active', 'created_time')
    search_fields = ('name', 'code', 'description')
    ordering = ('order', 'created_time')
    readonly_fields = ('created_time', 'updated_time')
    fieldsets = (
        ('基本信息', {
            'fields': ('name', 'code', 'stage_type', 'stage_percentage', 'base_amount_type', 'order', 'is_active')
        }),
        ('描述信息', {
            'fields': ('description',)
        }),
...
class OutputValueMilestoneAdmin(BaseModelAdmin):
    """产值里程碑管理"""
    list_display = ('name', 'code', 'stage', 'milestone_percentage', 'order', 'is_active', 'created_time')
    list_filter = ('stage', 'is_active', 'created_time')
    search_fields = ('name', 'code', 'description')
    ordering = ('stage', 'order', 'created_time')
    readonly_fields = ('created_time', 'updated_time')
    raw_id_fields = ('stage',)
    fieldsets = (
        ('基本信息', {
            'fields': ('stage', 'name', 'code', 'milestone_percentage', 'order', 'is_active')
        }),
        ('描述信息', {
            'fields': ('description',)
        }),
...
class OutputValueEventAdmin(BaseModelAdmin):
    """产值事件管理"""
    list_display = ('name', 'code', 'milestone', 'event_percentage', 'responsible_role_code', 'order', 'is_active', 'created_time')
    list_filter = ('milestone', 'responsible_role_code', 'is_active', 'created_time')
    search_fields = ('name', 'code', 'description', 'trigger_condition')
    ordering = ('milestone', 'order', 'created_time')
    readonly_fields = ('created_time', 'updated_time')
    raw_id_fields = ('milestone',)
    fieldsets = (
        ('基本信息', {
            'fields': ('milestone', 'name', 'code', 'event_percentage', 'order', 'is_active')
        }),
        ('触发配置', {
            'fields': ('trigger_condition', 'responsible_role_code')
        }),
        ('描述信息', {
            'fields': ('description',)
        }),
