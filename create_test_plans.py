...
"""
创建工作计划测试数据
使用方法: python create_test_plans.py
"""
import os
import sys
...
def main():
    ...
    try:
        ...
        print(f'负责人: {user.username}')
    except User.DoesNotExist:
        print(f'❌ 用户 13880399996 不存在')
        return
    ...
    try:
        ...
        print(f'公司: {company.name if company else "未设置"}')
        print(f'部门: {org_dept.name if org_dept else "未设置"}')
    except Exception as e:
        print(f'⚠️ 获取用户信息失败: {e}')
        company = Company.objects.first()
        org_dept = None
        if company:
            print(f'使用默认公司: {company.name}')
    ...
    for i, plan_data in enumerate(test_plans, 1):
        ...
        print(f'   类型: {plan.get_plan_type_display()}')
        print(f'   周期: {plan.get_plan_period_display()}')
